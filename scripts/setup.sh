...
# auth: Zach Hartwig
#
# desc: Setup Bash script for the ROOT-based ADAQAcquisition
#       package. The script sets up access for two different types of
#       users: 'users' run preinstalled binaries from /usr/local/adaq;
...
if [ ! "$#" -eq 1 ]; then
    echo -e "\nError! The ADAQAcquisition setup.sh script only accepts single cmd line arg!"
...
if [ "$1" ==  'usr' ]; then
    ...
    echo -e "ADAQAcquisition : Environment has been configured for ADAQAcquisition user!"
    ...
    echo -e "ADAQAcquisition : Environment has been configured for ADAQAcquisition developer!"
fi
